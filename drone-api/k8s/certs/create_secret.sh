...
kubectl delete secret -n "${K8S_NS}" mtls-drone-api-credential --ignore-not-found
kubectl create -n "${K8S_NS}" secret generic mtls-drone-api-credential \
  --from-file=tls.key=${DRONE_API_KEY} \
  --from-file=tls.crt=${DRONE_API_CRT} \
